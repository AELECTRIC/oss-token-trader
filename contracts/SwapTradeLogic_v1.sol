...
contract SwapTradeLogic_v1 is VersionLogic, SwapTrade {
    ...
    function SwapTradeLogic_v1(ContractNameService _cns) public VersionLogic (_cns, CONTRACT_NAME) {}
    ...
    function approve(address _tokenAddr, address _spender, uint _amount, uint _nonce, bytes _sign) public {
        require(TokenInterface(_tokenAddr).approveWithSign(_spender, _amount, _nonce, _sign));
    }
    ...
    function trade(
        ...
        require(TokenTrader(_traderAddr).trade(_makerTokenAddr, _makerAmount, _makerAddress, _takerTokenAddr, _takerAmount, _expiration, _tradeNonce, _takerSign, _makerSign));
    }
}
