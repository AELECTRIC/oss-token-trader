...
contract TokenTrader {
    ...
    function getNonce(address _makerAddress, address _takerAddress) public constant returns (uint nonce) {
    ...
    function trade(
        ...
        uint _expiration, uint _tradeNonce, bytes _takerSign, bytes _makerSign) 
        public returns (bool success) 
    {
        ...
        if (makerAddress != _makerAddress || _tradeNonce != getNonce(makerAddress, takerAddress) || _expiration < now) 
            return false;

        nonces[makerAddress][takerAddress]++;

        require(ERC20Interface(_makerTokenAddr).transferFrom(makerAddress, takerAddress, _makerAmount) && ERC20Interface(_takerTokenAddr).transferFrom(takerAddress, makerAddress, _takerAmount));
        return true;
    }
